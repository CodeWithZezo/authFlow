    ...
    <div className="flex items-start justify-between gap-4 mb-8">
      <div className="space-y-1">
        {breadcrumbs && breadcrumbs.length > 0 && (
          <nav className="flex items-center gap-1 text-xs text-text-muted mb-2">
...
export function EmptyState({ icon: Icon, title, description, action }: EmptyStateProps) {
  return (
    <div className="flex flex-col items-center justify-center py-20 px-8 text-center">
      {Icon && (
        <div className="mb-4 flex h-14 w-14 items-center justify-center bg-surface-2 border border-border">
...
export function ErrorMessage({ message, onClose }: ErrorMessageProps) {
  ...
  return (
    <div className="flex items-start gap-3 rounded-(--radius) bg-red-500/10 border border-red-500/20 p-3 text-sm text-red-400">
      <AlertCircle size={16} className="mt-0.5 shrink-0" />
      <span className="flex-1">{message}</span>
      {onClose && (
        <button onClick={onClose} className="shrink-0 hover:text-red-300 transition-colors">
          <X size={14} />
        </button>
      )}
    </div>
