...
describe("POST /api/v1/auth/login", () => {
  ...
  it("returns 200 with user data on valid credentials", async () => {
    const { user, plainPassword } = await createUser({ email: "login@example.com" });
  ...
  it("sets accessToken and refreshToken HTTP-only cookies on success", async () => {
    const { user, plainPassword } = await createUser({ email: "cookielogin@example.com" });

    const res = await request(app).post("/api/v1/auth/login").send({
      email: user.email, password: plainPassword,
    });
  ...
  it("does not include raw tokens in the response body", async () => {
    const { user, plainPassword } = await createUser({ email: "notoken@example.com" });
  ...
  it("is case-insensitive for email", async () => {
    const { user, plainPassword } = await createUser({ email: "casetest@example.com" });

    const res = await request(app).post("/api/v1/auth/login").send({
      email: "CASETEST@EXAMPLE.COM", password: plainPassword,
    });
  ...
  it("creates a new session document on each login", async () => {
    const { user, plainPassword } = await createUser({ email: "session@example.com" });

    // Login twice — should create two sessions
    await request(app).post("/api/v1/auth/login").send({ email: user.email, password: plainPassword });
    await request(app).post("/api/v1/auth/login").send({ email: user.email, password: plainPassword });
